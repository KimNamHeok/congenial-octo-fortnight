...
<h4>훈련생 등록</h4>
	<form action="${pageContext.request.contextPath }/07/formProcess" method="post">
	<ul>
		<li>
		...
		<li>
			최종학력 : <input type="radio" name="grade" value="G1">고졸
		...
		<li>
			...
			<select name="license" multiple>
				...
				<option value="L2">정보보안 기사</option>
	...
	<pre>
      input : type(입력 데이터의 종류, 입력 태그의 형태), name(parameter명, part명)
            value(초기값, file type의 경우 사용할 수 없음)
            disabled(전송 데이터에서 제외, boolean), required(필수입력 여부, boolean)
            readonly(값의 변경 가능 여부. boolean)
            입력값이 없어도 ""이 전송됨
      select : name(parameter명, part명)
             disabled(전송 데이터에서 제외, boolean), required(필수입력 여부, boolean)
             readonly(값의 변경 가능 여부. boolean)
             option으로 value가 결정됨
             option label(사용자에게 보여줄 텍스트), value(전송 데이터 값)
                   selected(select의 초기값 여부, boolean)
      
      input : type="checkbox | radio" 선택 여부(checked)로 값을 표현
      input : type="file" 선택한 파일의 2진 데이터 전송 구조 필요(multipart/*)
