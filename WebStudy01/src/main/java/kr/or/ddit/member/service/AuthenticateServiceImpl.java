...
public class AuthenticateServiceImpl implements AuthenticateService {
	private MemberDAO dao = new MemberDaoImpl();  // dao라는 필드에 MemberDaoImpl 객체를 직접 생성함으로써 DAO와의 의존관계를 형성
	...
	public boolean authenticate(MemberVO inputData) {	// inputData라는 MemberVO 타입의 객체는 클라이언트나 상위 계층으로부터 넘어온 회원정보를 담고 있음
		...
		if(saved!=null) {			// 조건문을 통해 조회된 회원 데이터가 존재하는지를 검사
			...
			boolean result = savedPass.equals(inputPass);	// 위에 둘 비교 같으면 T 틀리면 F
			if(result) {
