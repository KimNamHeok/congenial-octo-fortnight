...
function createForm({months, locales, zoneIds}){
	...
	let {element:yearInput, setValue:setYear} = 
			InputComponent({name:"year", type:"number"});
	...
	let {element:zoneInput, setValue:setZone} = 
			SelectComponent({name:"zone", metaData:zoneIds});
...
document.addEventListener("DOMContentLoaded", async ()=>{
	...
	// 최초 달력 랜더링, form 의 submit 이벤트 발생이 필요함.
	form.requestSubmit();
});
